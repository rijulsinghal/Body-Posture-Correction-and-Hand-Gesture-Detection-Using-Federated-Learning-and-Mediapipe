...
with mp_holisitic.Holistic(min_detection_confidence = 0.5, min_tracking_confidence = 0.5) as holistic:
    ...
    while cap.isOpened():
        ...
        # Obtaining Landmark Coordinates for Ardha Pincha Mayurasana
        ...
        # Calculating the Angles required for Ardha Pincha Mayurasana
        ...
        # Check if Ardha Pincha Mayurasana Conditions are met
        # # Check if Elbows are in correct position
        # # # Check if Left Elbow is making a right angle
        if (left_elbow_angle > 100) and (left_elbow_angle < 140):
            ...
            cv2.putText(frame, 'Elbow Angle: ' + str(round(left_elbow_angle, 1)), 
                        tuple(np.multiply([left_elbow_x, left_elbow_y], window_size).astype(int)),
                        2, 0.5, red, 1)
        # # # Check if Right Elbow is making a right angle
        if (right_elbow_angle > 100) and (right_elbow_angle < 140):
        ...
        if (left_hip_angle > 60) and (left_hip_angle < 100):
        ...
        if (right_hip_angle > 60) and (right_hip_angle < 100):
        ...
        cv2.imshow("Ardha Pincha Mayurasana Pose", frame)
